   ...
   and cExtractFieldsInstruction {base_group_op = baseGroupOp,
    ...
    and compileIfThenElse {condition, then_part, else_part} =
        let
            val lab1     = newLabel NONE
            val lab2     = newLabel NONE
            val _ = emt "#Compiling if statement condition\n" []
            val (res, _) = cSourceOp condition
        in
            M.emitJumpIfZero (emt, res, lab1, [res]);
    ...
    and compileDoWhile {condition = condition,
        ...
        in
            ...
            let
                val (res, _) = cSourceOp condition
            in
                M.emitJumpIfNotZero (emt, res, beginLab, [res]);
                M.emitLabel (emt, breakLab)
            end
        end
...
(*              emt "#Here is the again label.\n" []; *)
              ...
              M.emitLabel (emt, continueLab);
(*              emt "#Here is the continue label.\n" []; *)
...
(*              emt "#Here is the break label.\n" [] *)
    ...
    and compileBranchTrueStatement {decision_operand = decOper,
        ...
        let
            ...
            val (reg, _) = cSourceOp decOper
        in
            M.emitJumpIfNotZero(emt, reg, lab, [reg])
        end
    ...
    and compileBranchFalseStatement {decision_operand = decOper,
        ...
        let
            ...
            val (reg, _) = cSourceOp decOper
        in
            M.emitJumpIfZero(emt, reg, lab, [reg])
        end
    ...
    and compileStatement body =
        let
            val _ = emt "# Begining of Statement here.\n" []
