...
struct
   ...
   (* TODO: document function input, output *)

   (* Atomic type or strucure *)
   ...
   let
      ...
      fun appn 0 _ = ()
	| appn n func = (func (); appn (n - 1) func)

      (* print a string to the output rtl file *)
      fun emit s  = TextIO.output (outFile, s)

      (* print a formated string to the output rtl file *)
	 ...
	 handle F.BadFormat => (print fmt; raise F.BadFormat)
      ...
      (* function finds respective symbols in the SUIF symbols tables. *)
      local
	 ...
	 val fileBlocks = #file_blocks fsb
      ...
      (* find the appropriate register type for a data type. *)
      fun type2RegType t = #1 (B.getRegType (#value (findType t)))

      (* find the appropriate register type for a symbol. *)
	...
	| getSymName _ = raise (Fail "Failure in getSymName")
      ...
      (* called by declareObject and varAddressReg *)
	 ...
	 in
	    if is_local
	       then M.emitLocVariableDef
		  (emt, currentProc (), name, loc, regTyp, size)
	    else M.emitGloVariableDef (emt, name, loc, regTyp)
	 end

      (* called by compileFileBlock and compileScopeStatement *)
	...
	| declareObject ({symbol = Z.VariableEntry x, refname = r}, false) =
	 ...
	 in
	    emitVarDef (name, valOf (!r), (#value o findType) ty, false)
	 end
	...
	| declareObject ({symbol = Z.ProcedureEntry x, refname = r}, false) =
	 ...
	 in
	    emitVarDef (name, valOf (!r), ty, false)
	 end
	...
	| emitConstantReg _ = raise (Fail "Error in const to Reg")
      ...
      (* Emit symbol definition if it's not already defined *)
      fun defineSym var =
	 let
		...
		{key, address_taken = _, def = {name = vsym, type' = ty, ...},
		 is_local = is_local}, refname = r as (ref NONE)} =>
		  ...
		  val loc  = if is_local then newLocal() else newGlobal ()
	       in
		  emitVarDef (name, loc, (#value o findType) ty, is_local);
		  r := SOME loc
	       end
	...
	| compileConst (Z.FloatConstant str, ty as (Z.FloatingPointType _)) =
	 let
	...
	| compileVB (SOME (Z.UndefinedValueBlock _), typ) =
	 ...
	 in
      ...
      fun doGlobal (Z.BooleanType _, _, _, _, _) =
	 raise (Fail "Global boolean type encountered")

	| doGlobal (typ as Z.IntegerType x, sym, align, valueBlock, static) =
	    (emitVarPrelude (sym, align, static);
	     compileVB (valueBlock, typ))

	| doGlobal (typ as Z.UIntegerType x, sym, align, valueBlock, static) =
	    (emitVarPrelude (sym, align, static);
	     compileVB (valueBlock, typ))

	| doGlobal (typ as Z.FloatingPointType _, sym, align, valueBlock,
		    static) =
	    (emitVarPrelude (sym, align, static);
	     compileVB (valueBlock, typ))

	| doGlobal (typ as Z.EnumeratedType _, sym, align, valueBlock, static)
	    =
	    (emitVarPrelude (sym, align, static);
	     compileVB (valueBlock, typ))

	| doGlobal (typ as Z.PointerType _, sym, align, valueBlock, static) =
	    (emitVarPrelude (sym, align, static);
	     compileVB (valueBlock, typ))

	| doGlobal (typ as Z.ArrayType x, sym, align, NONE, static) =
	...
	| doGlobal (typ as (Z.ArrayType _), sym, align,
		    vb as SOME (Z.UndefinedValueBlock _), static) =
	    doGlobal(typ, sym, align, NONE, static)

	| doGlobal (typ as (Z.ArrayType _), sym, align,
		    vb as SOME (Z.MultiValueBlock _), static) =
	    (emitVarPrelude (sym, align, static);
	     compileVB (vb, typ))
	...
	| doGlobal (typ as (Z.ArrayType _), sym, align,
	    ...
	    in
	       emitVarPrelude (sym, align, static);
	       appn n (fn () => compileVB (blk, typ))
	    end

	| doGlobal (Z.ArrayType _, _, _, SOME _, _) =
	    raise (Fail "Assertion failure in doGlobal: ArrayType")

	| doGlobal (typ, sym, align, valueBlock as SOME
		    (Z.UndefinedValueBlock _), static) =
	    doGlobal(typ, sym, align, NONE, static)

	| doGlobal (typ as Z.GroupType x, sym, align, valueBlock as SOME _,
		    static) =
	    (emitVarPrelude (sym, align, static);
	     compileVB (valueBlock, typ))

	| doGlobal (typ as Z.GroupType x, sym, align, NONE, static) =
	    let
	       val (name, fromLabel) = getSymName sym
	       val siz  = case #bit_size x of
		  Z.Finite s => IntInf.toInt(s) div 8
	 ...
	 let
	    val align = (B.getAllignment typ) div 8
	    ...
	    val typ' = getTyp typ
	 in
	    doGlobal(typ', symRef, align, valueBlock, static)
	 end
	 ...
	 let
	    fun findArgs (symRef as {symbol = Z.VariableEntry x,
		  ...
		  val {def = {type' = type', value_block = vb, ...}, ...} = x
	       in
		  (symRef, (#value o findType) type', vb, static)
	       end
	 ...
	 in
      ...
      (*************************** Compile Expressions  **********************)
(*      fun compileCompOp (rd, r1 as Reg (r, _), oper, r2, kr) =
	 let
	    val lab   = newLabel NONE
	    val opStr = M.getRtlOper (oper, r)
	 in
	    M.zeroOut (emt, rd);
	    M.compareRegs (emt, r1, r2, opStr, kr);
	    M.jumpWhen opStr r1 (emt, lab);
	    M.addOne (emt, rd);
	    M.emitLabel (emt, lab)
	 end
	| compileCompOp _ = raise Match
*)

      fun compileComparisonOp (rd, r1 as Reg _, oper, r2 as Reg _, kr) =
	 let
	    val lab   = newLabel NONE
	 in
	    M.compileComparisonOp(emt, rd, r1, oper, r2, kr)
	 end
	| compileComparisonOp _ = raise Match
	...
	| compileBinOp (x as (Z.Is_equal_to | Z.Is_not_equal_to |
			      Z.Is_less_than | Z.Is_less_than_or_equal_to |
			      Z.Is_greater_than |
			      Z.Is_greater_than_or_equal_to),
			r1, r2, rd, kr) =
	    compileComparisonOp (rd, r1, x, r2, kr)
	...
	| compileBinaryExpr {binop = binop,
	 ...
	 let
            ...
            val regTyp  = type2RegType type_id
            val rd      = M.newReg regTyp
	 in
	 ...
	 in
      ...
      and compileCopyExpr {source = source, result_type = type_id} =
	 let
            val (rd, _)  = compileExpr source
	 ...
	 let
            ...
            val regTy  = type2RegType result_type
            val rd     = M.newReg regTy
	 ...
	 in
      ...
      and compileSsaPhiExpr {variables, result_type} =
	 raise B.Can'tDoItYet
	 ...
	 case x of
	  ...
	  |  (Z.CopyExpression expr) => compileCopyExpr expr
	  ...
	  |  (Z.SsaPhiExpression expr) => compileSsaPhiExpr expr
      ...
      (* We raise StrangeConditional on finding a value in a control-flow    *)
      (* context.                                                            *)

      (* compileBooleanExpr : compile a source op in a control-flow context  *)
      fun compileBooleanExpr emt condition {tlab, flab} =
	 (compileBoolean emt condition {tlab=tlab, flab=flab})
	 handle StrangeConditional =>
	    valToFlow emt (#1 (compileExpr condition)) {tlab=tlab, flab=flab}

      (* valToFlow : given a Boolean value in rd, translate it in            *)
      (* control-flow context                                                *)
      and valToFlow emt rd {tlab=NONE, flab=SOME f} =
	 M.emitJumpIfZero (emt, rd, f, [rd])
	| valToFlow emt rd {tlab=SOME t, flab=NONE} =
	 M.emitJumpIfNotZero (emt, rd, t, [rd])
	| valToFlow emt rd {tlab=SOME t, flab=SOME f} =
	 (M.emitJumpIfZero (emt, rd, f, [rd]);
	 ...
	 case expr
	    of Z.BinaryExpression {binop, source1, source2, result_type} =>
		   ...
		   let
		      ...
		      val (r2, _) = compileExpr source2
		      val rd      =
			 M.newReg (type2RegType result_type)
		   in
		      compileRelOp emt binop r1 r2 rd {tlab=tlab, flab=flab}
		   end
		 | Z.Logical_and =>
      ...
      (* context.                                                           *)
      and compileRelOp emt binop r1 r2 rd {tlab=SOME t, flab=NONE} =
         let val regTy = case r1 of
	                   Reg (rTy, _) => rTy
			 | _ => raise Fail "non-register reg1"
             val opStr = M.getRtlOper (binop, regTy)
         in
	    M.compareRegs (emt, r1, r2, opStr, [r1, r2]);
	    M.jumpWhen opStr r1 (emt, t)
         end
	| compileRelOp emt binop r1 r2 rd {tlab=NONE, flab=SOME f} =
         compileRelOp emt binop r1 r2 rd {tlab=SOME f, flab=NONE}

	| compileRelOp emt binop r1 r2 rd {tlab=SOME t, flab=SOME f} =
         (compileRelOp emt binop r1 r2 rd {tlab=SOME t, flab=NONE};
          M.emitUncondJump (emt, f))

	| compileRelOp _ _ _ _ _ _ = raise Fail "comparison fall-through"
	...
	| logicalOr _ _ _ _ = raise Fail "logical or with both falling through"
      ...
      fun compileEval {expressions} = app (ignore o compileExpr) expressions

      and compileStatementList {statements} = app compileStatement statements
	 ...
	 in
      ...
      and compileWhile {condition = condition, body = body,
	 ...
	 in
      ...
      and compileReturnStatement {return_value = NONE} = M.compReturn (emt, NONE)
	| compileReturnStatement {return_value = SOME(retArg)} =
	 let
	 ...
	 raise B.Can'tDoItYet
      ...
      and compileBranchStatement {decision_operand = decision, target} =
	 ...
	 M.emitLabel (emt, getSymLabel defined_label)
      ...
      and compileStoreVariableStatement {destination = d, value = v} =
	 let
	    val sym           = defineSym d
            val a             = compileSymAddress sym
	 ...
	 in
            (case body of
		Z.EvalStatement expr => compileEval expr
	      | Z.StatementList stats => compileStatementList stats
	      | Z.IfStatement ifte => compileIfThenElse ifte
	      | Z.WhileStatement wst => compileWhile wst
	      | Z.DoWhileStatement dwst => compileDoWhile dwst
	      | Z.ForStatement fst => compileForStatement fst
	      | Z.ScopeStatement scst => compileScopeStatement scst
	      | Z.MarkStatement => compileMarkStatement ()
	      | Z.VaStartStatement vast => compileVaStartStatement vast
	      | Z.VaStartOldStatement vaost =>
		   compileVaStartOldStatement vaost
	      | Z.VaEndStatement vaest => compileVaEndStatement vaest
	      | Z.StoreStatement stost => compileStoreStatement stost
	      | Z.ReturnStatement rest => compileReturnStatement rest
	      | Z.JumpStatement just => compileJumpStatement just
	      | Z.JumpIndirectStatement jist =>
		   compileJumpIndirectStatement jist
	      | Z.BranchStatement bst => compileBranchStatement bst
	      | Z.MultiWayBranchStatement mwbst =>
		   compileMultiWayBranchStatement mwbst
	      | Z.LabelLocationStatement lablst =>
		   compileLabelLocationStatement lablst
	      | Z.StoreVariableStatement stvst =>
		   compileStoreVariableStatement stvst
	      | Z.NopStatement => compileNopStatement ())
                before
