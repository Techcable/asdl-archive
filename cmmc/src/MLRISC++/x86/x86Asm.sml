 ...
 *
 * Log:
 * Revision 1.2  1999/04/21 00:33:45  leunga
 * Fixed up the x86 description somewhat.   I think now it generates the
 * correct assembly.
 *
 * Revision 1.1  1999/04/19 23:24:57  leunga
 * *** empty log message ***
 *
...
struct
  ...
  fun emit s = TextIO.output(stream.asmOutStream,s)
	...
	(emit "\timul\t";
	 ...
	 (emit
	  ...
	  app (fn I => (emit "\t"; emitInstr(I, regmap)))
	      (Shuffle.shuffle {regmap=rmap, tmp=tmp, dst=dst, src=src})

      | I.FCOPY{dst, src, tmp, ...} => 
	  app (fn I => (emit "\t"; emitInstr(I, regmap)))
	      (Shuffle.shufflefp{regmap=fmap, tmp=tmp, dst=dst, src=src})
