 ...
 * register allocator, with interprocedural spilling. 
 *)
functor InterProceduralMemoryRA
   (FrameInfo : RA_FRAME_INFO)
   (Flowgraph : RA_FLOWGRAPH 
      where type flowgraph = FrameInfo.flowgraph
   ) : RA_FLOWGRAPH =
struct
  ...
  let val {build, spill=spillMethod, 
      ...
      fun spillIt{graph as GRAPH{mode, bitMatrix, memRegs, spillLoc,
                                 copyTmps, ...}, nodes,
      ...
      let val member = BM.member(!bitMatrix)
           ...
           *)
          fun spillCoalescing nodesToSpill = 
          let 
              (* Find moves between two spilled nodes *)
              fun collectMoves([], mv') = mv'
                | collectMoves(NODE{movelist, color=ref(SPILLED _), ...}::ns, 
                               mv') =
           ...
           * Spill propagation.
           *)
          fun spillPropagation nodesToSpill =
          let exception SpillProp
              ...
              fun coalescingSavings([], pinned, sc) = (pinned, sc+sc)
                | coalescingSavings(MV{status=ref(CONSTRAINED | COALESCED),...}
                                    ::mvs, pinned, sc) = 
                  ...
                  let val NODE{number=d, color=dstCol, ...} = chase dst
                      ...
                      fun savings(x) =
                          if member(d, s) then
                              ...
                              coalescingSavings(mvs,pinned,sc+cost)
              ...
              fun insert([], worklist) = worklist
                | insert((node as NODE{color=ref PSEUDO, number, ...})::adj, 
                         worklist) =
              ...
              fun propagate([], spilled) = spilled
                | propagate((node as NODE{color as ref PSEUDO, 
                            ...
                            spilled) =
                  let val (pinned, savings) = 
                           coalescingSavings(!movelist, ~1, 0)
           ...
           * themselves however.
           *)
          fun spillColoring nodesToSpill = 
          let val proh     = A.array(length nodesToSpill, ~1)
              ...
              fun selectColor([], firstColor, currLoc) = ()
                | selectColor(NODE{color as ref(SPILLED ~1), number, adj, ...}
                              ::nodes,
                              firstColor, currLoc) = 
                  let fun neighbors [] = ()
                        | neighbors(n::ns) = 
                      ...
                      fun findColor(loc, startingPoint) = 
                          let val loc = if loc < firstColor 
                                        then !spillLoc + 1 else loc
                          in  if A.sub(proh, firstLoc - loc) <> number then loc 
