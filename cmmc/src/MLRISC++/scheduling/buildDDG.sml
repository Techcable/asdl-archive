...
functor SchedulerDDGBuilder
     (structure DDG : SCHEDULER_DDG
      ...
      structure SchedProps  : SCHEDULING_PROPERTIES
         sharing InsnProps.I = SchedProps.I = DDG.I = CFG.I
...
struct
   ...
   fun buildDDG{arch, cfg, numberOfInstructions, blockIdTbl} = 
   let val CFG as G.GRAPH cfg = cfg
       ...
       val {defUse, ...} = SchedProps.info arch
       ...
       val m_anti      = DDG.EDGE{l=0, r= ~1, d=DDG.MEM_ANTI}
       ...
       fun scanBlock{ops,defTbl,useTbl} =
       let fun addOutputAndAnti j (r,_) =  
           ...
           fun scan [] = ()
             | scan((i,DDG.NODE{defs, uses,...})::rest) =
       ...
       let val bid = !blockId (* block id *)
           ...
           let val node = (!nodeId, DDG.NODE{i=instr,b=bid,defs=defs,uses=uses})
           in  nodeId := !nodeId + 1;
               #add_node ddg node;
               node
           end 
           ...
           fun addLiveIn X = 
               ...
               in  app (addOutputAndAnti i) liveIn;
                   app (fn r => HA.update(defs, r, 
                        (i,DDG.EDGE{l=0,r=r,d=DDG.LIVEIN})::HA.sub(defs, r))) 
                      liveIn;
                   liveInNode
               end           
           ...
           fun addLiveOut X = 
               (case #exit_edges cfg X of
                 ...
                 let fun createLiveOutNode(liveOut) =
                     ...
                     (* Note: Don't bother adding edges between 
                      * livein->liveout 
                      *)
                     fun addLiveOut j r =
                         app (fn (i,DDG.EDGE{l,r,d=DDG.LIVEIN}) => () 
                               | (i,DDG.EDGE{l,r,...}) => 
           ...
           val _ = scanBlock{ops=ops, defTbl=defs, useTbl=uses}; 
