...
sig
   ...
   val showHI : cell -> string
   val showLO : cell -> string
   ...
   val showHIWithType : (cell * ty) -> string
   val showLOWithType : (cell * ty) -> string
...
struct
   ...
   structure MyCellsBasis = CellsBasis
      (type cellkind = mycellkind
       ...
       val FLOAT = FP
       val firstPseudo = 256
       val kinds = [GP, FP, CC, HI, LO, MEM, CTRL, UNKNOWN]
       val physical = [{from=0, to=31, kind=GP}, {from=32, to=63, kind=FP}, {from=64, to=63, kind=CC}, {from=64, to=64, kind=HI}, {from=65, to=65, kind=LO}, {from=66, to=65, kind=MEM}, {from=66, to=65, kind=CTRL}]
      )
   ...
   and offsetMEM = 66
   and offsetCTRL = 66
   val stackptrR = (29 + offsetGP)
   val frameptrR = (30 + offsetGP)
   val globalptrR = (28 + offsetGP)
   val asmTmpR = (1 + offsetGP)
   val fasmTmp = (30 + offsetFP)
   fun zeroReg GP = SOME (0 + offsetGP)
     | zeroReg _ = NONE
   fun showGPWithType (r, ty) = (fn (1, _) => "$at"
                                  | (28, _) => "$gp"
                                  | (29, _) => "$sp"
                                  | (30, _) => "$fp"
                                  | (r, _) => "$" ^ (Int.toString r)
   ...
   and showCCWithType (r, ty) = let
       ...
       in (fn (r, _) => "$" ^ (Int.toString r)
          ) (r, ty)
       end

   and showHIWithType (r, ty) = let
          val r = (if (r <= 64)
                 then (r - 64)
                 else r)
       in (fn _ => "$hi"
   ...
   fun showHI r = showHIWithType (r, 64)
   fun showLO r = showLOWithType (r, 64)
   ...
   fun toStringWithType GP = showGPWithType
     ...
     | toStringWithType HI = showHIWithType
     | toStringWithType LO = showLOWithType
